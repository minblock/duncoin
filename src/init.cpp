 ...
 */
static const char* BITCOIN_PID_FILENAME = "duncoind.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/duncoin-project/duncoin>";
    const std::string URL_WEBSITE = "<https://duncoin.org>";
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if duncoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if duncoin is started while in a temporary directory.\n",
